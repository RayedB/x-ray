...
//
  async getAndGatherRawData(url = this.baseUrl){ //acces à this._baseUrl par le getter

    try{
      const rawData = await axios.get(url); //1er call
      console.log(url);

      let rawDataArray = [];
      rawDataArray.push(rawData);

      const numberOfCalls = Math.ceil(rawData.data.count / 10); //nombre de fois qu'on va faire appel à l'api sur les differentes pages
  ...
  async refineData(url = this.baseUrl){
    try {
      const rawDataArray = await this.getAndGatherRawData(url);

      if(rawDataArray.length > 1){ //si plus d'une page de resultats dans rawData
        const refinedData = [];
...
class Specie extends Api {
  ...
  async getStarWarsSpeciesMoreThanTwoPeople(){
    try {
      ...
      console.log('getStarWarsSpeciesMoreThanTwoPeople ERROR : ', e);
    }
  }
...
class Character extends Api {
  ...
  async getStarWarsCharacterBySearch(){
    ...
    try {
      ...
      console.log('pas de nom spécifié');
      return
    }
    catch (error) {
  ...
  async aggregateHumanData(){
    try {
      ...
      // const refinedPlanets = await this.refineData('https://swapi.co/api/planets'); //ne fonctionne pas car besoin d'une instance de planet
      ...
      // const refinedFilms = await this.refineData('https://swapi.co/api/films'); //ne fonctionne pas car besoin d'une instance de film
